...
class MasterFundMonitoringFormTests(TestCase):
    def setUp(self):
        ...
        self.fund_source = FundSource.objects.create(
            name="Test Fund", annual_budget=1000
        )
        self.division = Division.objects.create(name="Test Division")
        self.mooe_cat = BreakdownCategory.objects.create(
            name="Test", code="MO001", is_active=True
        )
        # need a district for the NC
        self.district = District.objects.create(name="Test District", order=1)
        self.nc = NegosyoCenter.objects.create(
            name="Test NC", code="test_nc", district=self.district
        )
