...
from data_management_app.models import Staff, Supplier
...
def _log_import_activity(
    request, uploaded_file, data_type, result, skip_errors, sheet_name=None, skip_rows=0
):
...
def process_import(data_type, file_path, skip_errors, sheet_name=None, skip_rows=0):
    ...
    if data_type == "supplier":
        ...
        return process_bank_statement_import(
            file_path, skip_errors, sheet_name, skip_rows
        )
    elif data_type == "master_fund_monitoring":
        return process_master_fund_monitoring_import(
            file_path, skip_errors, sheet_name, skip_rows
        )
...
def process_supplier_import(file_path, skip_errors, sheet_name=None, skip_rows=0):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if "philgeps_registration" in row and pd.notna(
                row["philgeps_registration"]
            ):
                supplier_data["philgeps_registration"] = str(
                    row["philgeps_registration"]
                ).strip()
            else:
            ...
            supplier, created = Supplier.objects.update_or_create(
                ...
                defaults={k: v for k, v in supplier_data.items() if k != "supplier"},
            )
...
def process_staff_import(file_path, skip_errors, sheet_name=None, skip_rows=0):
    ...
    try:
        ...
        if file_path.endswith((".xlsx", ".xls")):
            ...
            try:
                df = pd.read_excel(
                    file_path, sheet_name=sheet_name, skiprows=skip_rows, header=None
                )
            except ValueError as e:
                # If sheet not found, try with None (first sheet)
                if "not found" in str(e).lower():
                    df = pd.read_excel(
                        file_path, sheet_name=0, skiprows=skip_rows, header=None
                    )
                else:
                    raise
        else:
        ...
        try:
            ...
            if "," in staff_name:
                ...
                if (
                    middle_idx >= 0
                    and middle_idx > 0
                    and middle_idx < len(name_words) - 1
                ):
            ...
            if not first_name or len(first_name) < 2:
                raise ValueError(
                    f'Invalid first name: "{first_name}" (minimum 2 characters required)'
                )
            if not last_name or len(last_name) < 2:
                raise ValueError(
                    f'Invalid last name: "{last_name}" (minimum 2 characters required)'
                )
...
def process_bank_statement_import(file_path, skip_errors, sheet_name=None, skip_rows=0):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            try:
                balance = Decimal(str(row["balance"]).replace(",", "")).quantize(
                    Decimal("0.01")
                )
            except Exception:
                raise ValueError(
                    f"Invalid balance '{row['balance']}'. Must be a number"
                )
            ...
            if debit > 0 and credit > 0:
                raise ValueError(
                    "Cannot have both debit and credit. Only one should have a value."
                )
            ...
            try:
                ...
                if "status" in stmt_data:
                    BankStatement.objects.filter(pk=statement.pk).update(
...
def process_master_fund_monitoring_import(
    file_path, skip_errors, sheet_name=None, skip_rows=0
):
        ...
        try:
            ...
            if "fund_source" in row and pd.notna(row["fund_source"]):
                try:
                    ...
                    fund_source = FundSource.objects.filter(
                        name__iexact=fs_value
                    ).first()
                    if not fund_source:
                        # Try partial match
                        fund_source = FundSource.objects.filter(
                            name__icontains=fs_value
                        ).first()
                    if fund_source:
            ...
            if "nc" in row and pd.notna(row["nc"]):
                try:
                    ...
                    if not nc:
                        # Try partial match
                        nc = NegosyoCenter.objects.filter(
                            name__icontains=nc_value
                        ).first()
                    if nc:
                        mfm_data["nc"] = nc
                except Exception:
            ...
            if "purchase_type" in row and pd.notna(row["purchase_type"]):
                try:
                    ...
                    # Try exact match first, then case-insensitive, then partial match
                    purchase_type = PurchaseType.objects.filter(
                        name__iexact=pt_value
                    ).first()
                    if not purchase_type:
                        # Try partial match
                        purchase_type = PurchaseType.objects.filter(
                            name__icontains=pt_value
                        ).first()
                    if purchase_type:
            ...
            if "payments" in row and pd.notna(row["payments"]):
                try:
                    mfm_data["payments"] = round_decimal(
                        Decimal(str(row["payments"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "downloads" in row and pd.notna(row["downloads"]):
                try:
                    mfm_data["downloads"] = round_decimal(
                        Decimal(str(row["downloads"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "cleared_date" in row and pd.notna(row["cleared_date"]):
                try:
                    if isinstance(row["cleared_date"], str):
                        ...
                        for date_format in date_formats:
                            try:
                                mfm_data["cleared_date"] = datetime.strptime(
                        ...
                        mfm_data["cleared_date"] = pd.to_datetime(
                            row["cleared_date"]
                        ).date()
                except Exception:
                    pass
            ...
            if "account_title" in row and pd.notna(row["account_title"]):
                try:
                    ...
                    account = ExpenseObject.objects.filter(
                        name__iexact=acc_value_clean
                    ).first()
                    if not account:
                        # Try partial match on cleaned value
                        account = ExpenseObject.objects.filter(
                            name__icontains=acc_value_clean
                        ).first()
                    if not account and acc_value != acc_value_clean:
                        # Try original value as fallback
                        account = ExpenseObject.objects.filter(
                            name__iexact=acc_value
                        ).first()
                    if not account and acc_value != acc_value_clean:
                        account = ExpenseObject.objects.filter(
                            name__icontains=acc_value
                        ).first()
                    if account:
            ...
            if "goods_5_percent" in row and pd.notna(row["goods_5_percent"]):
                try:
                    mfm_data["goods_5_percent"] = round_decimal(
                        Decimal(str(row["goods_5_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "services_5_percent" in row and pd.notna(row["services_5_percent"]):
                try:
                    mfm_data["services_5_percent"] = round_decimal(
                        Decimal(str(row["services_5_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass

            if "goods_services_3_percent" in row and pd.notna(
                row["goods_services_3_percent"]
            ):
                try:
                    mfm_data["goods_services_3_percent"] = round_decimal(
                        Decimal(str(row["goods_services_3_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "goods_1_percent" in row and pd.notna(row["goods_1_percent"]):
                try:
                    mfm_data["goods_1_percent"] = round_decimal(
                        Decimal(str(row["goods_1_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "services_2_percent" in row and pd.notna(row["services_2_percent"]):
                try:
                    mfm_data["services_2_percent"] = round_decimal(
                        Decimal(str(row["services_2_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "rental_5_percent" in row and pd.notna(row["rental_5_percent"]):
                try:
                    mfm_data["rental_5_percent"] = round_decimal(
                        Decimal(str(row["rental_5_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass
            ...
            if "prof_fee_10_percent" in row and pd.notna(row["prof_fee_10_percent"]):
                try:
                    mfm_data["prof_fee_10_percent"] = round_decimal(
                        Decimal(str(row["prof_fee_10_percent"]).replace(",", ""))
                    )
                except Exception:
                    pass

            # Optional: Expense Classification
            if "expense_classification" in row and pd.notna(
                row["expense_classification"]
            ):
                try:
                    ec_value = str(row["expense_classification"]).strip()
                    # Try exact match first, then case-insensitive, then partial match
                    expense_cat = ExpenseCategory.objects.filter(
                        name__iexact=ec_value
                    ).first()
                    if not expense_cat:
                        # Try partial match
                        expense_cat = ExpenseCategory.objects.filter(
                            name__icontains=ec_value
                        ).first()
                    if expense_cat:
                ...
                try:
                    ...
                    if not staff:
                        ...
                        staff = Staff.objects.filter(
                            first_name__icontains=staff_value
                        ).first()
                    if not staff:
                        # Try last_name
                        staff = Staff.objects.filter(
                            last_name__iexact=staff_value
                        ).first()
                    if not staff:
                        # Try partial match on last_name
                        staff = Staff.objects.filter(
                            last_name__icontains=staff_value
                        ).first()
                    if not staff:
                        ...
                        if len(name_parts) > 0:
                            ...
                            staff = Staff.objects.filter(
                                last_name__iexact=last_name_candidate
                            ).first()
                            if not staff:
                                staff = Staff.objects.filter(
                                    last_name__icontains=last_name_candidate
                                ).first()
                    if not staff:
                        ...
                        if len(name_parts) > 0:
                            ...
                            staff = Staff.objects.filter(
                                first_name__iexact=first_name_candidate
                            ).first()
                            if not staff:
                                staff = Staff.objects.filter(
                                    first_name__icontains=first_name_candidate
                                ).first()
            ...
            mfm, created = MasterFundMonitoring.objects.update_or_create(
                **lookup_fields,
                defaults={k: v for k, v in mfm_data.items() if k not in lookup_fields},
            )
            mfm.full_clean()
            mfm.save()
