...
class StaffForm(forms.ModelForm):
    ...
    def clean_first_name(self):
        ...
        if len(first_name) < 2:
            raise ValidationError(
                "First name must be at least 2 characters long.", code="min_length"
            )
        if len(first_name) > 100:
            raise ValidationError(
                "First name must not exceed 100 characters.", code="max_length"
            )
        try:
        ...
        try:
            ...
            raise ValidationError(
                "First name contains invalid content.", code="script_injection"
            )
    ...
    def clean_middle_initial(self):
        ...
        if len(middle_initial) > 5:
            raise ValidationError(
                "Middle initial must not exceed 5 characters.", code="max_length"
            )
        try:
            validate_letters_only(middle_initial)
        except ValidationError:
            raise ValidationError(
                "Middle initial must contain only letters.", code="invalid_chars"
            )
        try:
            validate_no_script_content(middle_initial)
        except ValidationError:
            raise ValidationError(
                "Middle initial contains invalid content.", code="script_injection"
            )
    ...
    def clean_last_name(self):
        ...
        if len(last_name) < 2:
            raise ValidationError(
                "Last name must be at least 2 characters long.", code="min_length"
            )
        if len(last_name) > 100:
            raise ValidationError(
                "Last name must not exceed 100 characters.", code="max_length"
            )
        try:
        ...
        try:
            ...
            raise ValidationError(
                "Last name contains invalid content.", code="script_injection"
            )
    ...
    def clean(self):
        ...
        if first_name and last_name and first_name.lower() == last_name.lower():
            raise ValidationError(
                "First name and last name must be different.", code="same_names"
            )
        return cleaned_data
...
class SupplierForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "philgeps_registration": forms.TextInput(
                attrs={
                    "class": "form-control",
            ...
            "propprietor": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Proprietor/Owner name"}
            ),
    ...
    def clean_supplier(self):
        ...
        if len(supplier) < 2:
            raise ValidationError(
                "Supplier name must be at least 2 characters long.", code="min_length"
            )
        if len(supplier) > 200:
            raise ValidationError(
                "Supplier name must not exceed 200 characters.", code="max_length"
            )
        try:
        ...
        try:
            ...
            raise ValidationError(
                "Supplier name contains invalid content.", code="script_injection"
            )
        ...
        if queryset.exists():
            raise ValidationError(
                "A supplier with this name already exists.", code="unique"
            )
    ...
    def clean_vat_status(self):
        ...
        if vat_status not in valid_choices:
            raise ValidationError(
                "Invalid VAT status selection.", code="invalid_choice"
            )
        return vat_status
    ...
    def clean_philgeps_registration(self):
        ...
        if len(philgeps) > 100:
            raise ValidationError(
                "PhilGEPS registration must not exceed 100 characters.",
                code="max_length",
            )
        try:
    ...
    def clean_address(self):
        ...
        if len(address) < 5:
            raise ValidationError(
                "Address must be at least 5 characters long.", code="min_length"
            )
        if len(address) > 500:
            raise ValidationError(
                "Address must not exceed 500 characters.", code="max_length"
            )
        try:
            validate_no_script_content(address)
        except ValidationError:
            raise ValidationError(
                "Address contains invalid content.", code="script_injection"
            )
    ...
    def clean_propprietor(self):
        ...
        if len(proprietor) < 2:
            raise ValidationError(
                "Proprietor name must be at least 2 characters long.", code="min_length"
            )
        if len(proprietor) > 200:
            raise ValidationError(
                "Proprietor name must not exceed 200 characters.", code="max_length"
            )
        try:
        ...
        try:
            ...
            raise ValidationError(
                "Proprietor name contains invalid content.", code="script_injection"
            )
    ...
    def clean_contact_number(self):
        ...
        try:
            ...
            raise ValidationError(
                "Please enter a valid Philippine phone number.", code="invalid_format"
            )
        return contact
...
class FundSourceBreakdownForm(forms.ModelForm):
    ...
    def clean_budget_amount(self):
        ...
        if budget_amount <= 0:
            raise ValidationError(
                "Budget amount must be greater than zero.", code="invalid"
            )
        try:
            validate_transaction_amount(budget_amount)
        except ValidationError:
            raise ValidationError(
                "Budget amount must be a valid positive number.", code="invalid_amount"
            )
    ...
    def clean(self):
        ...
        query = FundSourceBreakdown.objects.filter(
            fund_source=self.fund_source, category=category
        )
        ...
        total_breakdown = (
            FundSourceBreakdown.objects.filter(fund_source=self.fund_source)
...
class TaxTableForm(forms.ModelForm):
    purchase_type = forms.ModelChoiceField(
        ...
        widget=forms.Select(
            attrs={"class": "form-select", "required": True, "id": "id_purchase_type"}
        ),
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        used_purchase_types = TaxTable.objects.values_list(
            "purchase_type_id", flat=True
        )
        self.fields["purchase_type"].queryset = PurchaseType.objects.filter(
            is_active=True
        ).exclude(id__in=used_purchase_types)
        if self.instance.pk:
            self.fields["purchase_type"].queryset = PurchaseType.objects.filter(
                is_active=True
            ).exclude(id__in=used_purchase_types) | PurchaseType.objects.filter(
                id=self.instance.purchase_type_id
            )
    ...
    def clean_purchase_type(self):
        ...
        if queryset.exists():
            raise ValidationError(
                "A tax entry for this purchase type already exists.", code="unique"
            )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "name": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "Expense object name",
                    "required": True,
                }
            ),
