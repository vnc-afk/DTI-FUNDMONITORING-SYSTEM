...
class Division(models.Model):
    ...
    def clean(self):
        ...
        if self.id:
            duplicate = (
                Division.objects.filter(name__iexact=self.name)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
...
class Staff(models.Model):
    ...
    middle_initial = models.CharField(
        max_length=5, blank=True, validators=[validate_letters_only]
    )
...
class Supplier(models.Model):
    ...
    tin = models.CharField(
        max_length=50,
        validators=[validate_tin_format, validate_tin_numeric],
        blank=True,
        null=True,
    )
    ...
    philgeps_registration = models.CharField(
        max_length=100, blank=True, validators=[validate_alphanumeric_with_spaces]
    )
    ...
    def clean(self):
        ...
        self.contact_number = (
            sanitize_string_input(self.contact_number) if self.contact_number else ""
        )
        ...
        if self.id:
            duplicate = (
                Supplier.objects.filter(supplier__iexact=self.supplier)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
                raise ValidationError(
                    {"supplier": "A supplier with this name already exists."}
                )
...
class FundSource(models.Model):
    name = models.CharField(
        max_length=100,
        unique=True,
        validators=[validate_string_length(min_length=2, max_length=100)],
    )
    annual_budget = models.DecimalField(
        max_digits=15, decimal_places=2, validators=[validate_budget_amount]
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def clean(self):
        ...
        if self.id:
            duplicate = (
                FundSource.objects.filter(name__iexact=self.name)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
                raise ValidationError(
                    {"name": "A fund source with this name already exists."}
                )
...
class FundSourceBreakdown(models.Model):
    fund_source = models.ForeignKey(
        FundSource, on_delete=models.CASCADE, related_name="breakdowns"
    )
    ...
    budget_amount = models.DecimalField(
        max_digits=15, decimal_places=2, validators=[validate_transaction_amount]
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def clean(self):
        if not self.fund_source_id or not self.budget_amount:
            return
        total_breakdown = (
            FundSourceBreakdown.objects.filter(fund_source_id=self.fund_source_id)
            .exclude(id=self.id)
...
class ExpenseObject(models.Model):
    code = models.CharField(
        max_length=50,
        unique=True,
        validators=[validate_string_length(min_length=5, max_length=50)],
    )
    name = models.CharField(
        max_length=255,
    ...
    color = models.CharField(
        max_length=7, default="#3498db", validators=[validate_hex_color]
    )
    description = models.TextField(
        blank=True, null=True, validators=[validate_string_length(max_length=500)]
    )
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def clean(self):
        ...
        if self.id:
            duplicate = (
                ExpenseObject.objects.filter(code=self.code)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
...
class ExpenseCategory(models.Model):
    ...
    color = models.CharField(
        max_length=7, default="#95a5a6", validators=[validate_hex_color]
    )
    description = models.TextField(
        blank=True, null=True, validators=[validate_string_length(max_length=500)]
    )
    ...
    def clean(self):
        ...
        if self.id:
            duplicate = (
                ExpenseCategory.objects.filter(name__iexact=self.name)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
...
class District(models.Model):
    ...
    def clean(self):
        ...
        if not self.name or len(self.name.strip()) < 2:
            raise ValidationError(
                {"name": "District name must be at least 2 characters."}
            )
        if self.id:
            duplicate = (
                District.objects.filter(name__iexact=self.name)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
                raise ValidationError(
                    {"name": "A district with this name already exists."}
                )
...
class NegosyoCenter(models.Model):
    district = models.ForeignKey(
        District, on_delete=models.CASCADE, related_name="negosyo_centers"
    )
    ...
    def clean(self):
        ...
        if self.id:
            duplicate = (
                NegosyoCenter.objects.filter(code__iexact=self.code)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
...
class PurchaseType(models.Model):
    ...
    def clean(self):
        ...
        if self.id:
            duplicate = (
                PurchaseType.objects.filter(name__iexact=self.name)
                .exclude(id=self.id)
                .exists()
            )
            if duplicate:
                raise ValidationError(
                    {"name": "A purchase type with this name already exists."}
                )
