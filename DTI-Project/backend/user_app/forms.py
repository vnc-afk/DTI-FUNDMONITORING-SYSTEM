...
class UserAccountForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if len(username) < 3:
            raise ValidationError(
                "Username must be at least 3 characters long.", code="min_length"
            )

        if len(username) > 150:
            raise ValidationError(
                "Username must not exceed 150 characters.", code="max_length"
            )
    ...
    def clean_email(self):
        ...
        if existing.exists():
            raise ValidationError(
                "An account with this email already exists.", code="duplicate"
            )
