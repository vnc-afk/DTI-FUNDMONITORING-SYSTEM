    ...
    def __init__(self, *args, is_first_transaction=True, **kwargs):
        ...
        if not is_first_transaction:
            ...
            if not self.is_bound:
                if self.instance and self.instance.pk and self.instance.balance is not None:
                    self.fields['balance'].initial = self.instance.balance
                else:
                    last_transaction = BankStatement.objects.exclude(
                        id=self.instance.id if self.instance and self.instance.id else None
                    ).order_by('-date', '-created_at').first()
                    self.fields['balance'].initial = last_transaction.balance if last_transaction else Decimal('0.00')
