...
class BankAccount(models.Model):
	...
	def clean(self):
		...
		if self.id:
			duplicate_name = BankAccount.objects.filter(name__iexact=self.name).exclude(id=self.id).exists()
			if duplicate_name:
				raise ValidationError({'name': 'A bank account with this name already exists.'})
			duplicate_acc = BankAccount.objects.filter(account_number=self.account_number).exclude(id=self.id).exists()
			if duplicate_acc:
...
class BankStatement(ArchivableModel):
	...
	ARCHIVE_UPDATE_FIELDS = {'is_archived', 'archived_at', 'archived_by', 'archive_reason'}
	...
	debit = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True, default=0, validators=[validate_transaction_amount])
	credit = models.DecimalField(max_digits=15, decimal_places=2, blank=True, null=True, default=0, validators=[validate_transaction_amount])
	balance = models.DecimalField(max_digits=15, decimal_places=2)
	status = models.CharField(max_length=20, choices=CATEGORY_CHOICES, default='On Process', null=True, blank=True)
	created_at = models.DateTimeField(auto_now_add=True)
	updated_at = models.DateTimeField(auto_now=True)
	...
	def _calculate_balance(self):
		...
		previous_statements = BankStatement.objects.all_with_archived().exclude(id=self.id).order_by('date', 'created_at')
		if self.created_at:
			previous_on_date = previous_statements.filter(Q(date__lt=self.date) | Q(date=self.date, created_at__lt=self.created_at))
		else:
	...
	def clean(self):
		...
		if debit > 0 and credit > 0:
			raise ValidationError('Cannot have both debit and credit amounts in the same transaction.', code='both_debit_credit')
		if debit == 0 and credit == 0:
			raise ValidationError('At least one of debit or credit must be non-zero.', code='no_amount')
		if self.balance is not None and self.balance < 0:
			raise ValidationError('Balance cannot be negative. Please check the transaction amounts or opening balance.', code='negative_balance')

	def save(self, *args, **kwargs):
		update_fields = kwargs.get('update_fields')
		if update_fields is not None:
			update_field_set = set(update_fields)
			if update_field_set and update_field_set.issubset(self.ARCHIVE_UPDATE_FIELDS):
				return super().save(*args, **kwargs)

		first_transaction = BankStatement.objects.all_with_archived().exclude(id=self.id).order_by('date', 'created_at').first()
		...
		if is_first_transaction:
			subsequent_transactions = BankStatement.objects.all_with_archived().exclude(id=self.id).order_by('date', 'created_at')
