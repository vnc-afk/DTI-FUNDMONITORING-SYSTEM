...
def sync_cheque_status_to_monitoring(sender, instance, created, **kwargs):
    ...
    matching_records = MasterFundMonitoring.objects.filter(cheque_number=instance.check_number)
    status_mapping = {
        'Cleared': 'Cleared',
        'On Process': 'Pending',
    }
...
def sync_cheque_status_from_bank_statement(sender, instance, created, update_fields, **kwargs):
    ...
    if update_fields and (update_fields == {'cheque_status'} or update_fields == {'cheque_status', 'cleared_date'}):
        return

    if not instance.cheque_number:
        return

    bank_statement = BankStatement.objects.filter(check_number=instance.cheque_number).first()
    if bank_statement:
        status_mapping = {
            'Cleared': 'Cleared',
            'On Process': 'Pending',
        ...
        cleared_date = bank_statement.date if bank_statement.status == 'Cleared' else None
        
        # Only update if status or cleared_date has changed
        if instance.cheque_status != new_status or instance.cleared_date != cleared_date:
