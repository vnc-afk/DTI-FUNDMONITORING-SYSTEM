...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if count == 0:
            self.stdout.write(self.style.SUCCESS('No master fund monitoring records found.'))
            return
        
        self.stdout.write(
            self.style.WARNING(f'Found {count} master fund monitoring record(s) to delete.')
        )
