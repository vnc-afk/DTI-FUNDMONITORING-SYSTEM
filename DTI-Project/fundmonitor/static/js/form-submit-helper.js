...
class FormSubmissionHandler {
   ...
   * If no specific fields are configured, listens to all form inputs for changes
   */
  setupLivePreview() {
    if (!this.form || !this.enableLivePreview) {
      return;
    }

    // If specific detail fields are configured, listen to those
    if (this.detailFields && this.detailFields.length > 0) {
      this.detailFields.forEach(fieldName => {
        ...
        if (!field || typeof field.addEventListener !== 'function') {
          console.warn(`[WARNING] Field "${fieldName}" found but is not a valid element or doesn't support addEventListener`);
          return;
        }
        ...
        try {
          ...
          console.error(`[ERROR] Error attaching listeners to field "${fieldName}":`, e);
        }
      });
    } else {
      // No specific fields - listen to ALL form inputs
      const formElements = this.form.querySelectorAll(
        'input[name]:not([type="hidden"]):not([type="submit"]):not([type="button"]):not([type="reset"]), ' +
        'select[name], ' +
        'textarea[name]'
      );
      ...
      formElements.forEach(field => {
        try {
          ...
          console.warn(`[WARNING] Error attaching listener to field:`, e);
        }
      });
    }
  }
   ...
   */
  updateLivePreview() {
    // Capture current values (will capture all fields if detailFields is empty)
  ...
  /**
   * Capture ALL form field values automatically
   * @returns {object} Key-value pairs of all form field labels and values
   */
  captureAllFormDetails() {
    const details = {};
    ...
    // Track which fields we've already processed (for radio/checkbox groups)
    const processedFields = new Set();

    formElements.forEach(field => {
      const fieldName = field.name;

      // Skip if we've already processed this radio/checkbox group
      if ((field.type === 'radio' || field.type === 'checkbox') && processedFields.has(fieldName)) {
        return;
      }
      ...
      // Get label and format value
      let label = this.form.querySelector(`label[for="${field.id}"]`)?.textContent?.trim()
      ...
      try {
        if (field.type === 'checkbox') {
          ...
          value = (selectedOption?.textContent || selectedOption?.text || field.value || '').trim() || '(Empty)';
        } else {
          value = field.value || '(Empty)';
        }
      } catch (e) {
        console.warn(`[WARNING] Error reading value from field "${fieldName}":`, e);
      ...
      if (value && typeof value === 'string' && value !== '(Empty)' && value !== 'Not selected') {
        ...
        if (fieldName.toLowerCase().includes('date') && !isNaN(Date.parse(value))) {
          try {
    ...
    console.log(`[SUCCESS] Captured ${Object.keys(details).length} form fields automatically`);
    return details;
  }

  /**
   * Auto-capture form field values for detail display
   * @param {string[]} fieldNames - Array of form field names to capture (or empty for all)
  ...
  captureFormDetails(fieldNames) {
    ...
    // If no specific fields specified, capture all fields
    if (!fieldNames || fieldNames.length === 0) {
      return this.captureAllFormDetails();
    }
    ...
    fieldNames.forEach(fieldName => {
      ...
      if (!field || !field.tagName) {
        ...
        console.warn(`[WARNING] Form field not found or invalid: "${fieldName}"`);
        return;
      }
      ...
      if (field.type === 'radio') {
        ...
        try {
          ...
          console.warn(`[WARNING] Error reading radio button "${fieldName}":`, e);
          value = '(Error reading value)';
        }
      } else {
      ...
      if (value && typeof value === 'string') {
        try {
          ...
          console.warn(`[WARNING] Error formatting value for field "${fieldName}":`, e);
        }
      }
    ...
    if (notFoundFields.length > 0) {
      console.warn(`[WARNING] Could not find ${notFoundFields.length} fields: ${notFoundFields.join(', ')}`);
    }
    if (foundFields.length > 0) {
      console.log(`[SUCCESS] Found ${foundFields.length} fields: ${foundFields.join(', ')}`);
    }
  ...
  submit(event) {
    ...
    // If detail fields are specified, auto-capture them; otherwise capture all fields
  ...
  showConfirmationDialog() {
    ...
    // Capture details if not already done (will capture all fields if detailFields is empty)
    if (Object.keys(this.details).length === 0) {
      this.details = this.captureFormDetails(this.detailFields);
    }
    ...
    style.textContent = `
      ...
      .form-submission-dialog-body::-webkit-scrollbar {
        width: 6px;
      }
      .form-submission-dialog-body::-webkit-scrollbar-track {
        background: var(--bg-surface);
      }
      .form-submission-dialog-body::-webkit-scrollbar-thumb {
        background: var(--border-strong);
        border-radius: 3px;
      }
      .form-submission-dialog-body::-webkit-scrollbar-thumb:hover {
        background: var(--accent);
      }
      .form-submission-dialog-details {
        padding: 14px;
        background: var(--bg-surface);
        border: 1px solid var(--border);
  ...
  performSubmit(submitBtn) {
    if (this.onSubmit) {
      ...
      submitBtn.innerHTML = `<span style="margin-right: 6px;"><i class="fa-solid fa-hourglass-end"></i></span> Submitting...`;
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('[data-form-submit-confirm]').forEach(form => {
    ...
    const handler = new FormSubmissionHandler({
      ...
      icon: form.dataset.confirmIcon || '<i class="fa-solid fa-circle-info"></i>',
