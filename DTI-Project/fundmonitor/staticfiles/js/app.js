...
function loadExternalScriptIfNeeded(src) {
...
// AJAX-based page navigation with intelligent caching
// Uses pageLoader for automatic caching and optimal performance
function navigateToPage(url, event) {
    ...
    // Use the smart pageLoader that handles caching automatically
    if (typeof pageLoader !== 'undefined') {
        pageLoader.loadPage(url, {
            useCache: true,
            showSpinner: true,
            onProgress: (state) => {
                if (state === 'fetching_network') {
                    console.log('🔄 Fetching from server...');
                } else if (state === 'loaded_from_cache') {
        ...
        // Fallback to original behavior if pageLoader not available
        console.warn('pageLoader not available, using standard AJAX');
        loadPageAjax(url);
    }
...
// Original AJAX navigation (fallback)
function loadPageAjax(url) {
    ...
    .then(async html => {
        ...
        // Execute scripts from the loaded page in document order
        const scripts = newDoc.querySelectorAll('script');
        for (const script of scripts) {
            if (script.src) {
                const src = script.getAttribute('src');
                await loadExternalScriptIfNeeded(src);
                continue;
            }
