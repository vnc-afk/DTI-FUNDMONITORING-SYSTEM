...
function setupFundReportButtons() {
    ...
    if (exportButton) {
        exportButton.addEventListener("click", (event) => {
            ...
            const exportFn = fundFromWindow("exportFundReportAll");
...
function initFundReport(reportData) {
    ...
    if (summaryCards) {
        summaryCards.innerHTML = reportData.funds
            .map((fund, index) => {
            const budget = reportData.budgetData[fund.id];
            const disbursed = budget ? budget.total_disbursed : 0;
            const burPercent = budget ? fundAsNumber(budget.bur_percent).toFixed(1) : "0.0";
            return `
          <div class="scard report-row-disburse" style="animation-delay: ${index * 0.025}s">
            <div class="scard-label">${fund.name}</div>
            <div class="scard-amount">${formatCurrency(disbursed)}</div>
            <div class="scard-pct">${burPercent}% BUR</div>
          </div>
        `;
        })
            .join("");
...
function initializeFundReportIfReady() {
    const reportData = fundFromWindow("FUND_REPORT_DATA");
    if (reportData) {
        initFundReport(reportData);
    }
}
