...
def get_items_per_page(request, default=25):
    ...
    try:
        page_size = int(UserPreference.objects.only("items_per_page").get(user=user).items_per_page)
...
def notifications_enabled_for_user(user, default=True):
    ...
    try:
        return bool(UserPreference.objects.only("notifications_enabled").get(user=user).notifications_enabled)
...
def notifications_enabled_for_request(request, default=True):
    ...
    return notifications_enabled_for_user(getattr(request, "user", None), default=default)
